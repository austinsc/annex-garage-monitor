...
const static char SCREENSHOT_PATH[] = "/mnt/sda1/annex-garage-monitor/garageCam/";
const static char SCREENSHOT_FILETYPE[] = ".png";
const static char TIMESTAMP_CMD[] = "date +%r;Y%m%d-%r";
const static char SCREENSHOT_CMD[] = "wget http://192.168.0.28:8080/?action=snapshot -O ";
...
static const byte motorPins[GARAGES] = {5, 13};
...
//      Serial.println(digitalRead(sensorPins[motor]));
    ...
    // Sends the screenshot command to webcam
    // Note: Can only view feed on desktop at the moment
    else if (command == "screenshot") {
      Process screenshot;
      // Generate filename from timestamp
      String filename = client.readString();
      filename.trim();
      
      String path = SCREENSHOT_PATH + filename + SCREENSHOT_FILETYPE;
      Serial.println(SCREENSHOT_CMD + path);
      
      // Take screenshot
      screenshot.runShellCommand(SCREENSHOT_CMD + path);
